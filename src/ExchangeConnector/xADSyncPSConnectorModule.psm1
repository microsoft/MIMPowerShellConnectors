    ...
    process {
        try {
            ...
            if(-not $PSBoundParameters.ContainsKey('SearchBase')) {
                ...
                $SearchBase = Get-LdapObject -DistinguishedName RootDse @Params | Select-Object -ExpandProperty defaultNamingContext
            }
            ...
            $DirectoryEntry = Get-LDAPObject -DistinguishedName $SearchBase @Params 
...
function Add-xADSyncPSConnectorCSAnchor {
    [CmdletBinding()]
    param (
        [Parameter(Mandatory=$true, ValueFromPipeline = $true)]
        [Microsoft.MetadirectoryServices.CSEntryChange]
    ...
    process {
        $AnchorAttribute = [Microsoft.MetadirectoryServices.AnchorAttribute]::Create($Name,$Value)
        $InputObject.AnchorAttributes.Add($AnchorAttribute)
    }
